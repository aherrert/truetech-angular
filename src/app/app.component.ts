  ...
  ngOnInit(): void {
    ...
    this.router.events.subscribe(event => {
      if (event instanceof NavigationEnd) {
        const url = event.url;
        this.showHeader1 = !url.includes('/login') && !url.includes('/register') && !url.includes('/software'); // Mostrar el header en todas las rutas excepto '/about' y '/login'
        this.showHeader2 = url.includes('/login') || url.includes('/register') || url.includes('/software'); // Mostrar el header2 solo en '/login' y '/register'
      }
    });
  }
}
